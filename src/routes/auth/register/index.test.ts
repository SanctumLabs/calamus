...
import { addHeaders } from '@authentication/mock';
import {
	mockUserFindByEmail, createTokensSpy, USER_EMAIL, USER_PASSWORD
} from '../login/mock';

// import the mock for this file below all mock imports
import { mockUserCreate, bcryptHashSpy, USER_NAME, USER_PROFILE_PIC, cryptoRandomBytesSpy } from './mock';
...
describe('Register route', () => {
	...
	beforeEach(() => {
		...
		cryptoRandomBytesSpy.mockClear();
	});
	...
	it('Should send error when empty body is sent', async () => {
		...
		expect(cryptoRandomBytesSpy).not.toBeCalled();
	});
